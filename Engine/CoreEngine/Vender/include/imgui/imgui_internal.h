...
enum ImGuiInputFlagsPrivate_
{
    ...
    ImGuiInputFlags_RepeatUntilKeyModsChangeFromNone = 1 << 6,  // Stop repeating when released OR if keyboard mods are leaving the Player state. Allows going from Mod+Key to Key by releasing Mod.
...
struct ImGuiTabItem
{
    ...
    float               RequestedWidth;         // mWidth optionally requested by caller, -1.0f is unused
...
struct ImGuiTableColumn
{
    ...
    float                   InitStretchWeightOrWidth;       // Value passed to TableSetupColumn(). For mWidth it is a content width (_without padding_).
