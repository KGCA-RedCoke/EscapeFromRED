...
// - Test Engine & Automation ... https://github.com/ocornut/imgui_test_engine (Has suite, Has engine to automate your apps)
...
// [SECTION] ParamValue & Enumerations
...
// [SECTION] Helpers (ImGuiOnceUponAFrame, ImGuiTextFilter, ImGuiTextBuffer, ImGuiStorage, ImGuiListClipper, JMath Operators, ImColor)
...
// Helper Macros - IM_FMTARGS, IM_FMTLIST: BindShaderPipeline printf-style warnings to our formatting functions.
...
// Scalar data types
typedef unsigned int        ImGuiID;// A unique PinID used by widgets (typically the result of hashing a stack of string)
...
struct ImGuiPlatformIO;             // Multi-viewport support: interface for Platform/G_Device backends + viewports to render
...
// ParamValue (declared as int to allow using as flags without overhead, and to not pollute the top of this file)
// - Tip: Use your programming IDE navigation facilities on the names in the _central column_ below to find the actual flags/enum lists!
//   - In Visual Studio: CTRL+comma ("Edit.GoToAll") can follow symbols inside comments, whereas CTRL+F12 ("Edit.GoToImplementation") cannot.
//   - In Visual Studio w/ Visual Assist installed: ALT+G ("VAssistX.GoToImplementation") can also follow symbols inside comments.
//   - In VS Code, CLion, etc.: CTRL+click can follow symbols inside comments.
typedef int ImDrawFlags;            // -> enum ImDrawFlags_          // ParamValue: for ImDrawList functions
typedef int ImDrawListFlags;        // -> enum ImDrawListFlags_      // ParamValue: for ImDrawList instance
typedef int ImFontAtlasFlags;       // -> enum ImFontAtlasFlags_     // ParamValue: for ImFontAtlas build
typedef int ImGuiBackendFlags;      // -> enum ImGuiBackendFlags_    // ParamValue: for io.BackendFlags
typedef int ImGuiButtonFlags;       // -> enum ImGuiButtonFlags_     // ParamValue: for InvisibleButton()
typedef int ImGuiChildFlags;        // -> enum ImGuiChildFlags_      // ParamValue: for BeginChild()
typedef int ImGuiColorEditFlags;    // -> enum ImGuiColorEditFlags_  // ParamValue: for ColorEdit4(), ColorPicker4() etc.
typedef int ImGuiConfigFlags;       // -> enum ImGuiConfigFlags_     // ParamValue: for io.ConfigFlags
typedef int ImGuiComboFlags;        // -> enum ImGuiComboFlags_      // ParamValue: for BeginCombo()
typedef int ImGuiDockNodeFlags;     // -> enum ImGuiDockNodeFlags_   // ParamValue: for DockSpace()
typedef int ImGuiDragDropFlags;     // -> enum ImGuiDragDropFlags_   // ParamValue: for BeginDragDropSource(), AcceptDragDropPayload()
typedef int ImGuiFocusedFlags;      // -> enum ImGuiFocusedFlags_    // ParamValue: for IsWindowFocused()
typedef int ImGuiHoveredFlags;      // -> enum ImGuiHoveredFlags_    // ParamValue: for IsItemHovered(), IsWindowHovered() etc.
typedef int ImGuiInputFlags;        // -> enum ImGuiInputFlags_      // ParamValue: for Shortcut(), SetNextItemShortcut()
typedef int ImGuiInputTextFlags;    // -> enum ImGuiInputTextFlags_  // ParamValue: for InputText(), InputTextMultiline()
typedef int ImGuiItemFlags;         // -> enum ImGuiItemFlags_       // ParamValue: for PushItemFlag(), shared by all items
typedef int ImGuiKeyChord;          // -> ImGuiKey | ImGuiMod_XXX    // ParamValue: for IsKeyChordPressed(), Shortcut() etc. an ImGuiKey optionally OR-ed with one or more ImGuiMod_XXX values.
typedef int ImGuiPopupFlags;        // -> enum ImGuiPopupFlags_      // ParamValue: for OpenPopup*(), BeginPopupContext*(), IsPopupOpen()
typedef int ImGuiMultiSelectFlags;  // -> enum ImGuiMultiSelectFlags_// ParamValue: for BeginMultiSelect()
typedef int ImGuiSelectableFlags;   // -> enum ImGuiSelectableFlags_ // ParamValue: for Selectable()
typedef int ImGuiSliderFlags;       // -> enum ImGuiSliderFlags_     // ParamValue: for DragFloat(), DragInt(), SliderFloat(), SliderInt() etc.
typedef int ImGuiTabBarFlags;       // -> enum ImGuiTabBarFlags_     // ParamValue: for BeginTabBar()
typedef int ImGuiTabItemFlags;      // -> enum ImGuiTabItemFlags_    // ParamValue: for BeginTabItem()
typedef int ImGuiTableFlags;        // -> enum ImGuiTableFlags_      // ParamValue: For BeginTable()
typedef int ImGuiTableColumnFlags;  // -> enum ImGuiTableColumnFlags_// ParamValue: For TableSetupColumn()
typedef int ImGuiTableRowFlags;     // -> enum ImGuiTableRowFlags_   // ParamValue: For TableNextRow()
typedef int ImGuiTreeNodeFlags;     // -> enum ImGuiTreeNodeFlags_   // ParamValue: for TreeNode(), TreeNodeEx(), CollapsingHeader()
typedef int ImGuiViewportFlags;     // -> enum ImGuiViewportFlags_   // ParamValue: for ImGuiViewport
typedef int ImGuiWindowFlags;       // -> enum ImGuiWindowFlags_     // ParamValue: for Begin(), BeginChild()
...
// Multi-mSelection item index or identifier when using BeginMultiSelect()
// - Used by SetNextItemSelectionUserData() + and inside ImGuiMultiSelectIO structure.
// - Most users are likely to use this store an item INDEX but this may be used to store a POINTER/PinID as well. Read comments near ImGuiMultiSelectIO for details.
...
namespace ImGui
{
    ...
    IMGUI_API void          ShowIDStackToolWindow(bool* p_open = NULL); // create Stack Tool window. hover items with mouse to query information about the source of their unique PinID.
    ...
    // PinID stack/scopes
    // Read the FAQ (docs/FAQ.md or http://dearimgui.com/faq) for more details about how PinID are handled in dear imgui.
    // - Those questions are answered and impacted by understanding of the PinID stack system:
    //   - "Q: Why is my widget not reacting when I click on it?"
    //   - "Q: How can I have widgets with an empty label?"
    //   - "Q: How can I have multiple widgets with the same label?"
    // - Short version: PinID are hashes of the entire PinID stack. If you are creating widgets in a loop you most likely
    //   want to push a unique identifier (e.g. object pointer, loop index) to uniquely differentiate them.
    // - You can also use the "Label##foobar" syntax within widget label to distinguish them from each others.
    // - In this header file we use the "label"/"name" terminology to denote a string that will be displayed + used as an PinID,
    //   whereas "str_id" denote a string that is only used as an PinID and not normally displayed.
    IMGUI_API void          PushID(const char* str_id);                                     // push string into the PinID stack (will hash string).
    IMGUI_API void          PushID(const char* str_id_begin, const char* str_id_end);       // push string into the PinID stack (will hash string).
    IMGUI_API void          PushID(const void* ptr_id);                                     // push pointer into the PinID stack (will hash pointer).
    IMGUI_API void          PushID(int int_id);                                             // push integer into the PinID stack (will hash integer).
    IMGUI_API void          PopID();                                                        // pop from the PinID stack.
    IMGUI_API ImGuiID       GetID(const char* str_id);                                      // calculate unique PinID (hash of whole PinID stack + given parameter). e.g. if you want to query into ImGuiStorage yourself
    ...
    // Widgets: Trees
    // - TreeNode functions return true when the node is open, in which case you need to also call TreePop() when you are finished displaying the mOcTree node contents.
    IMGUI_API bool          TreeNode(const char* label);
    IMGUI_API bool          TreeNode(const char* str_id, const char* fmt, ...) IM_FMTARGS(2);   // helper variation to easily decorelate the id from the displayed string. Read the FAQ about why and how to use PinID. to align arbitrary text at the same level as a TreeNode() you can use Bullet().
    ...
    IMGUI_API bool          CollapsingHeader(const char* label, ImGuiTreeNodeFlags flags = 0);  // if returning 'true' the header is open. doesn't indent nor push on PinID stack. user doesn't have to call TreePop().
    ...
    // - Read comments near ImGuiMultiSelectIO for instructions/details and see 'Demo->Widgets->mSelection State & Multi-Select' for demo.
    // - TreeNode() is technically supported but... using this correctly is more complicated. You need some sort of linear/random access to your mOcTree,
    ...
    //  - IMPORTANT: Popup identifiers are relative to the current PinID stack, so OpenPopup and BeginPopup generally needs to be at the same level of the stack.
    ...
    IMGUI_API bool          BeginPopupContextItem(const char* str_id = NULL, ImGuiPopupFlags popup_flags = 1);  // open+begin popup when clicked on last item. Use str_id==NULL to associate the popup to previous item. If you want to use that on a non-interactive item such as Text() you need to pass in an explicit PinID here. read comments in .cpp!
    ...
    // Logging/Capture
    // - All text output from the interface can be captured into tty/file/clipboard. By default, mOcTree nodes are automatically opened during logging.
    ...
    IMGUI_API const ImGuiPayload*   GetDragDropPayload();                                                           // peek directly into the current payload from anywhere. returns NULL when drag and drop is finished or inactive. use ImGuiPayload::IsDataType() to Has for the payload type.
    ...
    IMGUI_API bool          IsItemActive();                                                     // is the last item active? (e.g. button being held, text field being edited. This will continuously return true while holding mouse button on an item. mItems that don't interact will always return false)
    ...
    IMGUI_API ImGuiID       GetItemID();                                                        // get PinID of last item (~~ often same ImGui::GetID(label) beforehand)
    ...
    IMGUI_API bool          IsRectVisible(const ImVec2& size);                                  // Has if rectangle (of given size, starting from cursor position) is visible / not clipped.
    IMGUI_API bool          IsRectVisible(const ImVec2& rect_min, const ImVec2& rect_max);      // Has if rectangle (in screen space) is visible / not clipped. to perform coarse clipping on user's side.
    ...
    IMGUI_API void          SetItemKeyOwner(ImGuiKey key);                                      // Set key owner to last item PinID if it is hovered or active. Equivalent to 'if (IsItemHovered() || IsItemActive()) { SetKeyOwner(key, GetItemID());'.
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_NoMouseInputs          = 1 << 9,   // Disable catching mouse, hovering Has with pass through.
...
// ParamValue for ImGui::BeginChild()
...
// ParamValue for ImGui::PushItemFlag()
...
// ParamValue for ImGui::InputText()
...
// ParamValue for ImGui::TreeNodeEx(), ImGui::CollapsingHeader*()
enum ImGuiTreeNodeFlags_
{
    ...
    ImGuiTreeNodeFlags_NoTreePushOnOpen     = 1 << 3,   // Don't do a TreePush() when open (e.g. for CollapsingHeader) = no extra indent nor pushing on PinID stack
    ImGuiTreeNodeFlags_NoAutoOpenOnLog      = 1 << 4,   // Don't automatically and temporarily open node when Logging is active (by default logging will automatically open mOcTree nodes)
...
// ParamValue for OpenPopup*(), BeginPopupContext*(), IsPopupOpen() functions.
...
enum ImGuiPopupFlags_
{
    ...
    ImGuiPopupFlags_AnyPopupId              = 1 << 10,  // For IsPopupOpen(): ignore the ImGuiID parameter and Has for any popup.
    ImGuiPopupFlags_AnyPopupLevel           = 1 << 11,  // For IsPopupOpen(): search/Has at any level of the popup stack (default Has in the current level)
    ImGuiPopupFlags_AnyPopup                = ImGuiPopupFlags_AnyPopupId | ImGuiPopupFlags_AnyPopupLevel,
};

// ParamValue for ImGui::Selectable()
...
// ParamValue for ImGui::BeginCombo()
enum ImGuiComboFlags_
{
    ...
    ImGuiComboFlags_WidthFitPreview         = 1 << 7,   // mWidth dynamically calculated from preview contents
    ImGuiComboFlags_HeightMask_             = ImGuiComboFlags_HeightSmall | ImGuiComboFlags_HeightRegular | ImGuiComboFlags_HeightLarge | ImGuiComboFlags_HeightLargest,
};

// ParamValue for ImGui::BeginTabBar()
...
// ParamValue for ImGui::BeginTabItem()
...
// ParamValue for ImGui::IsWindowFocused()
...
// ParamValue for ImGui::IsItemHovered(), ImGui::IsWindowHovered()
...
enum ImGuiHoveredFlags_
{
    ...
    ImGuiHoveredFlags_Stationary                    = 1 << 13,  // Require mouse to be stationary for style.HoverStationaryDelay (~0.15 sec) _at least one time_. After this, can move on same item/window. Using the stationary Has tends to reduces the need for a long delay.
...
// ParamValue for ImGui::DockSpace(), shared/inherited by child nodes.
...
enum ImGuiDockNodeFlags_
{
    ...
    //ImGuiDockNodeFlags_NoCentralNode              = 1 << 1,   //       // Disable Central FNode (the node which can stay empty)
    ImGuiDockNodeFlags_NoDockingOverCentralNode     = 1 << 2,   //       // Disable docking over the Central FNode, which will be always kept empty.
    ImGuiDockNodeFlags_PassthruCentralNode          = 1 << 3,   //       // Enable passthru dockspace: 1) DockSpace() will render a ImGuiCol_WindowBg background covering everything excepted the Central FNode when empty. Meaning the host window should probably use SetNextWindowBgAlpha(0.0f) prior to Begin() when using this. 2) When Central FNode is empty: let inputs pass-through + won't display a DockingEmptyBg background. See demo for details.
...
// ParamValue for ImGui::BeginDragDropSource(), ImGui::AcceptDragDropPayload()
enum ImGuiDragDropFlags_
{
    ...
    ImGuiDragDropFlags_SourceNoHoldToOpenOthers     = 1 << 2,   // Disable the behavior that allows to open mOcTree nodes and collapsing header by holding over them while dragging a source item.
    ...
    ImGuiDragDropFlags_AcceptBeforeDelivery         = 1 << 10,  // AcceptDragDropPayload() will returns true even before the mouse button is released. You can then call IsDelivery() to Has if the payload needs to be delivered.
...
// ParamValue for Shortcut(), SetNextItemShortcut(),
...
enum ImGuiInputFlags_
{
    ...
    // ParamValue for SetNextItemShortcut()
...
enum ImGuiBackendFlags_
{
    ...
    ImGuiBackendFlags_RendererHasVtxOffset  = 1 << 3,   // Backend G_Device supports ImDrawCmd::VtxOffset. This enables output of large meshes (64K+ vertices) while still using 16-bit indices.
    ...
    ImGuiBackendFlags_RendererHasViewports  = 1 << 12,  // Backend G_Device supports multiple viewports.
};
...
// ParamValue for InvisibleButton() [extended in imgui_internal.h]
...
// ParamValue for ColorEdit3() / ColorEdit4() / ColorPicker3() / ColorPicker4() / ColorButton()
...
// ParamValue for DragFloat(), DragInt(), SliderFloat(), SliderInt() etc.
// We use the same sets of flags for DragXXX() and SliderXXX() functions as the features are the same and it makes it easier to swap them.
...
// ParamValue for ImGui::BeginTable()
...
// ParamValue for ImGui::TableSetupColumn()
...
// ParamValue for ImGui::TableNextRow()
...
struct ImGuiStyle
{
    ...
    ImGuiDir    WindowMenuButtonPosition;   // Side of the collapsing/docking button in the title bar (Player/Left/Right). Defaults to ImGuiDir_Left.
    ...
    float       IndentSpacing;              // Horizontal indentation when e.g. entering a mOcTree node. Generally == (FontSize + FramePadding.x*2).
    float       ColumnsMinSpacing;          // Minimum horizontal spacing between two columns. Preferably > (FramePadding.x + 1).
    float       ScrollbarSize;              // mWidth of the vertical scrollbar, Height of the horizontal scrollbar.
    ...
    ImVec2      DisplayWindowPadding;       // BindShaderPipeline to regular windows: amount which we enforce to keep visible when moving near edges of your screen.
    ImVec2      DisplaySafeAreaPadding;     // BindShaderPipeline to every windows, menus, popups, tooltips: amount where we avoid displaying contents. Adjust if you cannot see the edges of your screen (e.g. on a TV where scaling Has not been configured).
...
struct ImGuiIO
{
    ...
    // Tools to Has correct Begin/End and BeginChild/EndChild behaviors.
    ...
    // Optional: Platform/G_Device backend name (informational only! will be displayed in About Window) + User data for backend/wrappers to store their own stuff.
...
struct ImGuiWindowClass
{
    ...
    ImGuiID             FocusRouteParentWindowId;   // PinID of parent window for shortcut focus route evaluation, e.g. Shortcut() call from Parent Window will succeed when this window is focused.
...
// We use it to e.g. store collapse state for a mOcTree (Int 0/1)
// This is optimized for efficient lookup (dichotomy into a contiguous buffer) and rare insertion (typically tied to user interactions aka max once a frame)
// You can use it as custom user storage for temporary values. Declare your own storage if, for example:
// - You want to manipulate the open/close state of a particular sub-mOcTree in your interface (mOcTree node uses Int 0/1 to store their state).
...
struct ImGuiStorage
{
    ...
    // Obsolete: use on your own storage if you know only integer are being stored (open/close all mOcTree nodes)
...
// **Player of the ImGui API are using ImColor directly but you can use it as a convenience to pass colors in either ImU32 or ImVec4 formats. Explicitly cast to ImU32 or ImVec4 if needed.
struct ImColor
{
    ImVec4          Value;
...
// - Refer to 'Demo->Widgets->mSelection State & Multi-Select' for demos using this.
...
// ParamValue for BeginMultiSelect()
enum ImGuiMultiSelectFlags_
{
    ...
    ImGuiMultiSelectFlags_SelectOnClick         = 1 << 13,  // BindShaderPipeline selection on mouse down when clicking on unselected item. (Default)
    ImGuiMultiSelectFlags_SelectOnClickRelease  = 1 << 14,  // BindShaderPipeline selection on mouse release when clicking an unselected item. Allow dragging an unselected item without altering selection.
    //ImGuiMultiSelectFlags_RangeSelect2d       = 1 << 15,  // Shift+mSelection uses 2d geometry instead of linear sequence, so possible to use Shift+up/down to select vertically in grid. Analogous to what BoxSelect does.
    ImGuiMultiSelectFlags_NavWrapX              = 1 << 16,  // [Temporary] Enable navigation wrapping on X axis. Provided as a convenience because we don't have a design for the general Nav API for this yet. When the more general feature be public we may obsolete this flag in favor of new one.
};

// Main IO structure returned by BeginMultiSelect()/EndMultiSelect().
// This mainly contains a list of selection requests.
// - Use 'Demo->Tools->Debug Log->mSelection' to see requests as they happen.
...
struct ImGuiSelectionBasicStorage
{
    ...
    ImGuiID         (*AdapterIndexToStorageId)(ImGuiSelectionBasicStorage* self, int idx);      // e.g. selection.AdapterIndexToStorageId = [](ImGuiSelectionBasicStorage* self, int idx) { return ((MyItems**)self->UserData)[idx]->PinID; };
    int             _SelectionOrder;// [Internal] Increasing counter to store selection order
    ImGuiStorage    _Storage;       // [Internal] mSelection set. Think of this as similar to e.g. std::set<ImGuiID>. Prefer not accessing directly: iterate with GetNextSelectedItem().

    // Methods
    ImGuiSelectionBasicStorage();
    IMGUI_API void  ApplyRequests(ImGuiMultiSelectIO* ms_io);   // BindShaderPipeline selection requests coming from BeginMultiSelect() and EndMultiSelect() functions. It uses 'items_count' passed to BeginMultiSelect()
...
struct ImGuiSelectionExternalStorage
{
    ...
    IMGUI_API void  ApplyRequests(ImGuiMultiSelectIO* ms_io);   // BindShaderPipeline selection requests by using AdapterSetItemSelected() calls
};
...
// Draw state is not reset by default because they are many perfectly useful way of altering render state (e.g. changing shader/blending settings before an Image call).
...
struct ImDrawCmd
{
    ...
    ImTextureID     TextureId;          // 4-8  // User-provided texture PinID. Set by user in ImfontAtlas::SetTexID() for fonts or passed to Image*() functions. Ignore if never using images or multiple fonts atlas.
...
// ParamValue for ImDrawList functions
...
// ParamValue for ImDrawList instance. Those are set automatically by ImGui:: functions from ImGuiIO settings, and generally not manipulated directly.
...
struct ImDrawList
{
    ...
    ImDrawListFlags         Flags;              // ParamValue, you may poke into these to adjust anti-aliasing settings per-primitive.
...
struct ImFontAtlasCustomRect
{
    ...
    unsigned int    GlyphID;        // Input    // For custom font glyphs only (PinID < 0x110000)
...
// ParamValue for ImFontAtlas build
...
//  - Mouse cursor shapes for software cursor rendering (unless setting 'ParamValue |= ImFontAtlasFlags_NoMouseCursors' in the font atlas).
...
struct ImFontAtlas
{
    ...
    IMGUI_API const ImWchar*    GetGlyphRangesJapanese();               // Default + Hiragana, Katakana, Half-mWidth, mSelection of 2999 Ideographs
    ...
    int                         PackIdMouseCursors; // Custom texture rectangle PinID for white pixel and mouse cursors
    int                         PackIdLines;        // Custom texture rectangle PinID for baked anti-aliased lines
...
struct ImFont
{
    ...
    float                       EllipsisWidth;      // 4     // out               // mWidth
...
// ParamValue stored in ImGuiViewport::ParamValue, giving indications to the platform backends.
enum ImGuiViewportFlags_
{
    ...
    ImGuiViewportFlags_NoRendererClear          = 1 << 8,   // Platform Window: G_Device doesn't need to clear the framebuffer ahead (because we will fill it entirely).
...
struct ImGuiViewport
{
    ...
    ImDrawData*         DrawData;               // The ImDrawData corresponding to this viewport. Valid after Draw() and until the next call to NewFrame().

    // Platform/Backend Dependent Data
    // Our design separate the G_Device and Platform backends to facilitate combining default backends with each others.
    // When our create your own backend for a custom engine, it is possible that both G_Device and Platform will be handled
...
// - Application:  In your main loop, call ImGui::UpdatePlatformWindows(), ImGui::RenderPlatformWindowsDefault() after EndFrame() or Draw().
...
struct ImGuiPlatformIO
{
    ...
    //
    // The functions are designed so we can mix and match 2 imgui_impl_xxxx files, one for the Platform (~window/input handling), one for G_Device.
    // Custom engine backends will often provide both Platform and G_Device interfaces and so may not need to use all functions.
    // Platform functions are typically called before their G_Device counterpart, apart from Destroy which are called the other way.
    ...
    void    (*Platform_SwapBuffers)(ImGuiViewport* vp, void* render_arg);   // . . . R .  // (Optional) Call Draw/SwapBuffers (platform side! This is often unused!). 'render_arg' is the value passed to RenderPlatformWindowsDefault().
    float   (*Platform_GetWindowDpiScale)(ImGuiViewport* vp);               // N . . . .  // (Optional) [BETA] FIXME-DPI: DPI handling: Return DPI scale for this viewport. 1.0f = 96 DPI.
    void    (*Platform_OnChangedViewport)(ImGuiViewport* vp);               // . F . . .  // (Optional) [BETA] FIXME-DPI: DPI handling: Called during Begin() every time the viewport we are outputting into changes, so backend Has a chance to swap fonts to adjust style.
    int     (*Platform_CreateVkSurface)(ImGuiViewport* vp, ImU64 vk_inst, const void* vk_allocators, ImU64* out_vk_surface); // (Optional) For a Vulkan G_Device to call into Platform code (since the surface creation needs to tie them both).

    // (Optional) G_Device functions (e.g. DirectX, OpenGL, Vulkan)
    ...
    void    (*Renderer_SwapBuffers)(ImGuiViewport* vp, void* render_arg);   // . . . R .  // (Optional) Call Draw/SwapBuffers. 'render_arg' is the value passed to RenderPlatformWindowsDefault().
    ...
    // Viewports list (the list is updated by calling ImGui::EndFrame or ImGui::Draw)
