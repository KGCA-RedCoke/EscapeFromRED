    ...
    for (int32_t i = 0; i < trackSize; ++i)
    {
        ...
        for (int32_t j = 0; j < keySize; ++j)
        {
            ...
            Utils::Serialization::Serialize_Primitive(&track->TransformKeys.RotationKeys[j].Value,
                                                      sizeof(FVector4),
                                                      FileStream);
            Utils::Serialization::Serialize_Primitive(&track->TransformKeys.ScaleKeys[j].Time, sizeof(float),
                                                      FileStream);
...
bool JAnimationClip::DeSerialize_Implement(std::ifstream& InFileStream)
{
    ...
    Utils::Serialization::DeSerialize_Primitive(&mSourceSamplingInterval, sizeof(mSourceSamplingInterval),
                                                InFileStream);
